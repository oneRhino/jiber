...
class RedmineClubhouseUsersController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        if (!RedmineClubhouseUser::where('clubhouse_name', Config::get('clubhouse.clubhouse_user'))->first()) {
            $user = new RedmineClubhouseUser();
            $user->clubhouse_name = Config::get('clubhouse.clubhouse_user');
            $user->redmine_names = "[]";
            $user->save();

            $request->session()->flash('alert-success', 'All users have been imported successfully!');
        } else {
            $request->session()->flash('alert-warning', 'Users already imported!');
        }
