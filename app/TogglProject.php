...
class TogglProject extends Toggl
{
    ...
    /**
     * TogglProject belongs to ClubhouseProject
     */
    public function clubhouseProject()
    {
        return $this->belongsTo('App\ClubhouseProject', 'clubhouse_id');
    }
    ...
    public static function getAllByUserID($user_id, $orderBy = 'name', $sort = 'ASC')
    {
        return self::where(array('toggl_projects.user_id' => $user_id))
            ->leftJoin('clubhouse_projects', 'toggl_projects.clubhouse_id', '=', 'clubhouse_projects.id')
            ->leftJoin('toggl_workspaces', 'workspace_id', '=', 'toggl_workspaces.id')
            ->leftJoin('toggl_clients', 'client_id', '=', 'toggl_clients.id')
            ->select(array(
                'toggl_projects.id',
                'toggl_projects.name',
                'toggl_projects.active',
                'toggl_workspaces.name as workspace_name',
                'toggl_clients.name as client_name',
                'clubhouse_projects.clubhouse_name as clubhouse_name'
            ))
            ->orderBy('client_name')
            ->orderBy('name')
            ->get();
